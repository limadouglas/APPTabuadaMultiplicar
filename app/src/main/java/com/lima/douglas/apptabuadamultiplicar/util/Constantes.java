package com.lima.douglas.apptabuadamultiplicar.util;

public class Constantes {
    public static final String BD_NOME = "BD_RECORDES"; //nome do banco de dados.
    public static final int BD_VERSION = 1;         // versao do banco de dados.
    public static final String APP_KEY = "6f0f6e900ac2fc0f797cb849cf66628c2f8e9a01eccf474e"; // numero da chave do appodeal.
    public static boolean TESTEAPPODEAL = false; // ativar modo teste de propaganda.
    public static int QTD_PROPAGANDA = 4; // ativar propaganda.
    public static int PROPAGANDA = 0; // ativar propaganda.
}
